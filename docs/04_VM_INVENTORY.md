...
| 2026-05-10 | **🔐 Phase 6E enforcement track SEALED.** All 3 VMs now use rotated 24-char unique passwords for SSH, MariaDB root, and ERPNext Administrator (per VM, no shared credentials). Production MariaDB root recovered via `--skip-grant-tables` safe mode after paste-corruption set the value to unknown (lesson L55 in `19_PHASE_6E_CLOSURE.md`). Laptop keyring service `infrabeat-vm-creds` is the canonical credential source (9 keys, all 24 chars). Pre-rotation passwords are now dead — historical commits in this now-public repo retain those values but they no longer authenticate anywhere. | `19_PHASE_6E_CLOSURE.md` |
| 2026-05-10 | **🟢 Phase 6E.4+ debt retirement: VMs unchanged.** No SSH, MariaDB, ERPNext, FAC, or systemd state changed on Dev / Staging / Production during the debt-retirement track. All VM credentials remain rotated (per Phase 6E.1.5) and stored in OS keyring service `infrabeat-vm-creds` on the laptop. **New OS keyring service `infrabeat-erp-master` added on laptop** for the Fernet master key (per Phase 6E.8 / PR #27 `909f4d5`), separating it from the OAuth-token namespace `infrabeat-erp`. The laptop now hosts three distinct keyring services: `infrabeat-erp` (CLI runtime OAuth tokens), `infrabeat-vm-creds` (operator SSH/MariaDB/Admin credentials), `infrabeat-erp-master` (Fernet master key). VMs themselves require no keyring changes. | `20_PHASE_6E_DEBT_RETIREMENT_CLOSURE.md` |
